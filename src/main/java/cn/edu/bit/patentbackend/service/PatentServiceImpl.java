...
//        List<Integer> list = Arrays.asList(1, 2);
    ...
    public BasicSearchResponse proSearch(String expression, Integer curPage, Integer perPage) {
        ...
        SearchHits hits = null;
        try {
            hits = patentRepository.basicSearch(searchRequest);
        } catch (IOException e) {
        ...
        long totalHits = hits.getTotalHits().value;
        int pageNum = (int)totalHits / perPage;
        BasicSearchResponse response = new BasicSearchResponse(curPage, totalHits, pageNum, perPage, expression, "","pro", results);
        return response;
    }
